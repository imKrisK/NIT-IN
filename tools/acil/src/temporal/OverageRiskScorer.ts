...
export interface OverageRiskResult {
  ...
  daysUntilExhaustion: number | null;  // null = won't exhaust before reset
  projectedOverageCost: number;        // USD estimate if no action taken
}

export class OverageRiskScorer {
  private _overageCostPerUnit: number;  // $ per request or $ per credit unit

  constructor(overageCostPerUnit: number = 0.04) {
  ...
  private _result(
    ...
    let label: OverageRiskResult['label'] = 'NONE';
    if (score >= 0.95) label = 'CERTAIN';
    else if (score >= 0.70) label = 'HIGH';
    else if (score >= 0.40) label = 'MODERATE';
    else if (score >= 0.15) label = 'LOW';
