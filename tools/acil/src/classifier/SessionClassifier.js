...
exports.SessionClassifier = void 0;
...
class SessionClassifier {
    ...
    classify(signals) {
        ...
        // ── Priority 1: AGENTIC ────────────────────────────────────────────────
        const agenticMatches = signals.toolCallSignatures.filter(s => AGENTIC_TOOL_SIGNATURES.includes(s.toLowerCase()));
        if (agenticMatches.length > 0 || this._containsAny(q, ['agent', 'run all', 'do everything', 'autonomously'])) {
            return {
                sessionType: types_1.SessionType.AGENTIC,
                confidence: 0.92,
        ...
        if (signals.newFileCount >= 3 || (archKeywords.length >= 2)) {
            ...
            return {
                sessionType: types_1.SessionType.ARCHITECTURE,
                confidence: conf,
                signals: [
        ...
        if (signals.hasErrorContext || this._containsAny(q, ['error', 'bug', 'fix', 'fail', 'crash', 'exception', 'why does', 'why is'])) {
            return {
                sessionType: types_1.SessionType.DEBUGGING,
        ...
        if (docKeywords.length >= 1) {
            return {
                sessionType: types_1.SessionType.DOCUMENTATION,
                confidence: 0.80,
        ...
        if (signals.existingFileSimilarity >= 0.70) {
            return {
                sessionType: types_1.SessionType.BOILERPLATE,
                confidence: 0.78,
        ...
        if (signals.contextRatio >= 0.80 && this._containsAny(q, ['review', 'check', 'look at', 'what do you think', 'is this correct', 'feedback'])) {
            return {
                sessionType: types_1.SessionType.REVIEW,
                confidence: 0.74,
...
exports.SessionClassifier = SessionClassifier;
