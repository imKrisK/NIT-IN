...
class ContradictionDetector {
    ...
    detect(source, responseText) {
        ...
        const resolution = bestContra >= this._config.blockThreshold ? 'block' :
    ...
    _contradictionScore(kw, text, prior) {
    ...
    _classifyConflict(textA, textB) {
        const combined = (textA + ' ' + textB).toLowerCase();
        if (/sql|postgres|mongo|mysql|redis|dynamo|sqlite/.test(combined))
            return 'architecture';
        if (/jwt|oauth|session|cookie|auth|security|encrypt/.test(combined))
            return 'security';
        if (/import|require|package|library|dependency|npm|pip/.test(combined))
            return 'dependency';
        if (/function|method|variable|class|interface|type/.test(combined))
            return 'naming';
        return 'approach';
    }
}
